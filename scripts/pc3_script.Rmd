...
The PC3 samples were filtered from dataset, to compare hypoxia vs normoxia. Normoxia is set as reference and DESeq2 was ran. The genes are ordered based on adjusted p-values
```{r}
cell_type <- "lncap"
...
The gene regulation under hypoxia condition in PC3 cell lines, based on which volcano plot is created. Genes were categorized as upregulated, downregulated, or not significant based on fold change and adjusted p-value.
```{r}
...
in order to understand broad biological responses in PC3 cells under hypoxia
...
###Hallmark GSEA – Hypoxia Response
The plot to show which Hallmark programs are enriched in PC3 cells under hypoxia. Pathways are sorted by NES, and bars are colored by significance (padj < 0.05).
